...
//------------------------get methods
app.get('/', (req, res) => {
	const filepath = StaticPath.join(__dirname,'index.html');
	res.sendFile(filepath);
});

app.get('/dashboard', (req, res) => {
	res.render(StaticPath.join(__dirname+'/dash/index'));
  });
...
app.get('/manage/add-user', (req, res) => {
	res.render(StaticPath.join(__dirname+'/manage/add-user'));
  });
//------------------------post methods
//login 
	 ...
	 if (email && password) {
		connection.query('SELECT * FROM users WHERE email = ? AND pass = ?', [email, password], (error, results, fields)=>{
			...
			if (results.length > 0) {
				...
				res.render(StaticPath.join(__dirname+'/'),{message:'Invalid Email or Password'});
			}			
			res.end();
		});
	} else {
		res.render(StaticPath.join(__dirname+'/'),{message:'Please check all the input'});
		res.end();
	}
	//res.json(req.body);
});
